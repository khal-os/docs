...
<Tabs>
  ...
  <Tab title="Full-stack">
    A pack with a frontend surface and a backend service image. The `permissions` list names the capabilities the pack uses; the `backend` block points the platform at the container image and chart.
...
<Frame>
  ![Annotated khal-app.json with arrows from each field to the runtime component it configures — id to launcher, permissions to capability gate, app kind to shell loading, backend to the Kubernetes workload.](/images/packs/khal-app-json-annotated.png)
</Frame>
