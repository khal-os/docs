...
<Steps>
  ...
  <Step title="Release source is reviewed">
...
On the FDE side, this means: do a good job on `khal-app.json` (accurate permissions, clear `env[]`, real description), prove `khal install ... --dry-run`, and your app can land cleanly in the target catalog.

See [Customer install](/release/customer-install) for the customer-side handoff contract — what you hand over after review/release.
...
```text
TASK: Use the Khal OS docs page `what-is-khal-os.mdx` (What is Khal OS?) to guide an FDE through the workflow, run only safe verification commands first, and return evidence with blockers.
