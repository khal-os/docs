...
<Steps>
  <Step title="Push">
    CI validates the app repo/ref and, when the backend is present, tags/publishes the service image and chart according to that repo's workflow.
  </Step>
...
```text
TASK: Use the Khal OS docs page `patterns/full-stack-pack.mdx` (Full-stack pack) to guide an FDE through the workflow, run only safe verification commands first, and return evidence with blockers.
