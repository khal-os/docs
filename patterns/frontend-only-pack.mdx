...
<Steps>
  <Step title="Push to dev">
    CI validates the app repo and records the reviewed git ref; no frontend npm package is published.
  </Step>
  <Step title="Merge to main">
    CI publishes `@latest`. No Docker step; no Helm step.
  </Step>
  <Step title="Install into an instance">
    A customer admin installs the reviewed git ref through the platform install flow. The shell loads the built frontend bundle on next launch.
  </Step>
</Steps>
