...
The KHAL-owned AppKit installer ships the public `khal` CLI. Install it, verify the version/template bundle, then scaffold locally.
...
<Note>
  Use another channel only when a release owner explicitly asks you to validate an unreleased CLI fix. Fresh FDE installs should use the default official installer path.
</Note>
...
<Callout type="info">
  New scaffolds should use `khal new`. Existing legacy packs can keep working, but do not start a new FDE pack from a GitHub template unless a maintainer explicitly names that template.
</Callout>
...
```text
TASK: Use the Khal OS docs page `getting-started/your-first-pack.mdx` (Your first pack) to guide an FDE through the workflow, run only safe verification commands first, and return evidence with blockers.
