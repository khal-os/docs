...
## 1. Install and verify the `khal` CLI

`@khal-os/app-kit` ships the `khal` command used throughout this guide. Until the auth-fixed CLI is promoted from npm `next` to `latest`, fresh FDE workstations must install the documented release channel explicitly:

```bash
npm install -g @khal-os/app-kit@next
khal --version
```

If you are connecting to a KhalOS instance from a headless shell, verify the auth surface before you scaffold or install packs:

```bash
khal login --no-browser --instance http://localhost:8888
khal auth status
khal whoami
khal auth logout
```

Replace `http://localhost:8888` with your assigned instance URL when you are not using a local dev instance. Do not paste the login URL, device code, or token output into tickets, docs, or chat.

## 2. Scaffold with `khal new`
