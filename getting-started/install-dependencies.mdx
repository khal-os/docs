...
The `khal` binary comes from `@khal-os/app-kit`. Until npm `latest` carries the auth-fixed release, use the documented `next` channel for fresh FDE installs:
...
khal --version   # should report 1.0.24 or newer
```
